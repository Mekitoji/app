...
var ApprovedApps = require('../../../models/CISEU/gkbaseApproved');
var Cal = require('../../../models/CISEU/calendar');
var ApprovedCal = require('../../../models/CISEU/calendarForApprovedApps');
...
module.exports = function (app) {
  ...
  //get all, raw data
  app.get('/api/ciseu/gk/all', function (req, res) {
  ...
  app.get('/api/ciseu/gk', function (req, res) {
    ...
    Apps.find(function (err, app) {
      ...
      log.info(new Date() + '  - GET /API/CISEU/GK');
    });
  });

  app.get('/api/ciseu/gk/notReviewed', function (req, res) {
  ...
  //rejected filter
  app.get('/api/ciseu/gk/rejected', function (req, res) {
    ...
    Apps.find(function (err, app) {
      ...
      log.info(new Date() + '  - GET /API/CISEU/GK/REJECTED');
    });
  });

  //approved filter
  app.get('/api/ciseu/gk/approved', function (req, res) {
  ...
  //outdated filter
  app.get('/api/ciseu/gk/outdated', function (req, res) {
    ...
    Apps.find(function (err, app) {
      ...
      log.info(new Date() + '  - GET /API/CISEU/GK/OUTDATED');
    });
  });


  app.get('/api/ciseu/gk/:app_id', function (req, res) {
  ...
  app.post('/api/ciseu/gk', function (req, res) {
    ...
    Apps.create({
      ...
      Apps.find({
        ...
        cal.save(function (err, data) {
          if (err) res.send(err);
          res.json(data);
          log.info(new Date() + '  - POST /API/CISEU/GK/' + data.appId);
        });
      });
    });
  });
  ...
  // delete a user
  app.delete('/api/ciseu/gk/:app_id', function (req, res) {
    Apps.remove({
  ...
  //update a user
  app.put('/api/ciseu/gk/:app_id', function (req, res) {
        ...
        .exec(function(err, cal) {
          ...
          if(cal && cal.storage.length !== 0 && !appOutdated && tvStatus !== 'Not Reviewed') {
            ...
            if(lCalc) {
              ...
              Rate.getRegion('CISEU', function(err, data) {
                if(err) console.error(err);
                if(!data) {
                  Rate.addRegion('CISEU', function(err, data) {
                    if(err) return console.error(err);
                    addPassToMonth(data);
                  });
                } else {
